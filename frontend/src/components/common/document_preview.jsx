...
const DocumentPreview = ({ fileUrl, fileName, fileToUpload }) => {
  ...
  const checkForOriginalDocx = async (url) => {
    try {
      ...
      if (fileUrl && typeof fileUrl === 'object' && fileUrl.originalDocUrl) {
        return fileUrl.originalDocUrl;
      }
  ...
  useEffect(() => {
    ...
    const processDocxFile = async () => {
      try {
        if (['doc', 'docx'].includes(fileExtension)) {
          ...
          const originalDocxUrl = await checkForOriginalDocx(fileUrl);
    ...
    // If it's a PDF file, use direct iframe embedding
    if (fileExtension === 'pdf' && fileUrl) {
      console.log("Using direct PDF URL for preview:", fileUrl);
      setPdfUrl(fileUrl);
      setLoading(false);
      return;
    }

    // First try to handle as original DOCX if possible
    processDocxFile().then(handled => {
      if (handled) return;

      // Handle other cases if not handled as original DOCX
      ...
      if (fileUrl) {
        if (['doc', 'docx'].includes(fileExtension)) {
          ...
          const fetchAndProcessDocx = async () => {
            try {
              console.log("Attempting to fetch and process DOCX:", fileUrl);
              ...
              if (typeof fileUrl === 'string' && fileUrl.toLowerCase().includes('.pdf')) {
              ...
              try {
                const actualUrl = typeof fileUrl === 'string' ? fileUrl :
    ...
    return () => {
      if (blobUrl && typeof blobUrl === 'string' && typeof fileUrl === 'string' && blobUrl !== fileUrl) {
        URL.revokeObjectURL(blobUrl);
      }
    };
  }, [fileUrl, fileToUpload, fileExtension, fileName]);
  ...
  const renderPreview = () => {
    ...
    if ((fileExtension === 'pdf' || pdfUrl) && (pdfUrl || fileUrl)) {
      return (
        <iframe
          src={`${pdfUrl || fileUrl}#toolbar=0&navpanes=0&scrollbar=0`}
          className="w-full h-full border-none"
          title="PDF Preview"
        />
      );
    ...
    // For image files
    if (['jpg', 'jpeg', 'png', 'gif', 'webp'].includes(fileExtension) && (blobUrl || fileUrl)) {
      return (
