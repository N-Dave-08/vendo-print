...
export const printPdfDirect = async (filePath, printerName, isColor = true) => {
  try {
    ...
    const psScript = [
      ...
      '        ',
      '        # Check for both 32-bit and 64-bit Ghostscript',
      '        $gs32Path = "C:\\Program Files (x86)\\gs\\gs*\\bin\\gswin32c.exe"',
      '        $gs64Path = "C:\\Program Files\\gs\\gs*\\bin\\gswin64c.exe"',
      '        ',
      '        $gsExe = $null',
      '        # Try 32-bit first on 32-bit systems',
      '        if ([System.Environment]::Is64BitOperatingSystem -eq $false) {',
      '            $gsExe = Get-ChildItem -Path $gs32Path -ErrorAction SilentlyContinue | Select-Object -First 1 -ExpandProperty FullName',
      '        } else {',
      '            # On 64-bit systems, try 64-bit first then fallback to 32-bit',
      '            $gsExe = Get-ChildItem -Path $gs64Path -ErrorAction SilentlyContinue | Select-Object -First 1 -ExpandProperty FullName',
      '            if (-not $gsExe) {',
      '                $gsExe = Get-ChildItem -Path $gs32Path -ErrorAction SilentlyContinue | Select-Object -First 1 -ExpandProperty FullName',
      '            }',
      '        }',
